...
for filename in html_files:
    ...
    # 1. Strip the old footer
    content = re.sub(r'<footer.*?>.*?</footer>', '', content, flags=re.DOTALL)
    
    # 2. Strip the sticky beta-cta-bar at the bottom
    content = re.sub(r'<div class="beta-cta-bar">.*?</div>\s*</body>', '</body>', content, flags=re.DOTALL)
    
    # 3. Strip any <script src="app.js"></script> before </body> so we don't have duplicates
    content = re.sub(r'<script src="app\.js"></script>', '', content)
    
    # 4. Replace the old waitlist CTA (if it was somehow left)
    content = re.sub(r'<div class="beta-cta reveal-on-scroll" id="beta-registration">.*?</div>', '', content, flags=re.DOTALL)
    
    # 5. Insert waitlist and footers exactly at the end of <main> and before </body>
    # The structure was: </main> (then old footer, old scripts) </body>
    content = content.replace('</main>', waitlist_banner_and_footers)
    # waitlist_banner_and_footers contains </main> at the end of the waitlist banner, then footers, then </body>
    # Wait, the structure was <main class="reading-pane"> ... </main>
    # So I will replace </main> \s* </body> with waitlist_banner_and_footers
    content = re.sub(r'</main>\s*</body>', waitlist_banner_and_footers, content, flags=re.DOTALL)
    
    # 6. Apply TOC leading zero replacements
    for old, new in toc_replacements.items():
        content = content.replace(old, new)
        
    # 7. Inject Back to Noetica Home button at end of sidebar
    if 'class="sidebar-back-btn"' not in content:
        content = content.replace('</nav>\n    </aside>', sidebar_back_btn + '\n    </nav>\n    </aside>')
        
    # 8. Replace index.html main cards with 8 Core Questions
