...
avatar_new = '<img src="creator.png" alt="Sk Anish Md." class="creator-avatar" style="width: 60px; height: 60px;">'
...
for filename in html_files:
    ...
    # 1. Update avatar
    content = re.sub(r'<div class="creator-avatar">.*?</div>', avatar_new, content, flags=re.DOTALL)
    
    # 2. Update socials
    content = re.sub(r'<div class="footer-socials">.*?</div>', socials_new, content, flags=re.DOTALL)
    
    with open(filename, 'wb') as f:
        f.write(content.encode('utf-8'))

# Update CSS for avatar object-fit
with open('index.css', 'rb') as f:
    css = f.read().decode('utf-8')

css = css.replace('font-size: 1.2rem;', 'font-size: 1.2rem;\n    object-fit: cover;\n    border: 2px solid var(--color-border);')
